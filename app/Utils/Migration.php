...
//                    } else if (substr($list, 0, 12) == "CREATE TABLE"){
                        ...
                        $table = explode(".",$list);
                        $_classname = trim(substr(trim($table[1]),0,(strlen($table[1])-2)));
                        ...
                        if (substr(trim($list), 0, 11) == "PRIMARY KEY"){
                            ...
                            if (!empty($explode) && (sizeof($explode) > 0)){
    ...
    public static function getMigrationField($obj, $indices){
        ...
        if ($obj){
            ...
            $index = "N";
            if (!empty($indices) && (sizeof($indices) > 0)){
            ...
            } else if ($obj->type == "TINYINT"){
                if ($obj->width > 0){
                    $ret = "\$table->boolean('".$obj->name."',".$obj->width.")";
                } else {
                    $ret = "\$table->boolean('".$obj->name."')";
                }
            ...
            if (!empty($array) && (sizeof($array) > 0)){
                ...
                foreach($array as $value){
                    if ($value->table["name"] == strtolower($table)){
                        if (!empty($value->table["fields"]) && (sizeof($value->table["fields"]) > 0)){
